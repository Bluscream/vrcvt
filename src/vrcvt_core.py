...
from vrcvt_runner import VRCTestRunner
...
def run_wmf_test(proton_bin, prefix_dir, wmf_exe, url, env_vars, retries=1):
    """Wrapper function delegating single test execution to VRCTestRunner class."""
    runner = VRCTestRunner(proton_bin=proton_bin, prefix_dir=prefix_dir, env_vars=env_vars, wmf_exe=wmf_exe)
    return runner.run_test(url, timeout=10, retries=retries)
