#!/usr/bin/env bash
# lock.sh - Lock VRChat Tools folder and binaries with read-only permissions to prevent VRChat from replacing them
...
if $TARGET_YTDLP && [ -f "$TOOLS_DIR/yt-dlp.exe" ]; then
    chmod 555 "$TOOLS_DIR/yt-dlp.exe"
    echo "🔒 Locked yt-dlp.exe (read-only)"
fi

if $TARGET_DENO && [ -f "$TOOLS_DIR/deno.exe" ]; then
    chmod 555 "$TOOLS_DIR/deno.exe"
    echo "🔒 Locked deno.exe (read-only)"
fi

if $TARGET_FFMPEG && [ -f "$TOOLS_DIR/ffmpeg.exe" ]; then
    chmod 555 "$TOOLS_DIR/ffmpeg.exe"
    echo "🔒 Locked ffmpeg.exe (read-only)"
fi
